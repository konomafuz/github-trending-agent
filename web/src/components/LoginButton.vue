<template>
  <div class="flex items-center">
    <!-- Not Authenticated -->
    <button
      v-if="!authStore.isAuthenticated"
      @click="handleLogin"
      :disabled="authStore.loading"
      class="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gray-900 rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50"
    >
      <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        <path fill-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.008-.866-.013-1.7-2.782.603-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.463-1.11-1.463-.908-.62.069-.608.069-.608 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.087 2.91.831.092-.646.35-1.086.636-1.336-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0112 6.836c.85.004 1.705.114 2.504.336 1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" clip-rule="evenodd"/>
      </svg>
      <span v-if="authStore.loading">登录中...</span>
      <span v-else>GitHub 登录</span>
    </button>

    <!-- Authenticated -->
    <div v-else class="flex items-center gap-3">
      <div class="flex items-center gap-2">
        <img
          :src="authStore.user!.avatar_url"
          :alt="authStore.user!.name"
          class="w-8 h-8 rounded-full"
        />
        <span class="text-sm font-medium text-gray-900">{{ authStore.user!.name }}</span>
      </div>
      <button
        @click="authStore.logout()"
        class="px-3 py-1 text-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100 rounded transition-colors"
      >
        退出
      </button>
    </div>
  </div>
</template>

<script setup lang="ts">
import { onMounted } from 'vue'
import { useAuthStore } from '../stores/auth'

const authStore = useAuthStore()

onMounted(() => {
  authStore.init()
})

async function handleLogin() {
  const token = prompt('请输入 GitHub Personal Access Token\n\n需要权限: gist\n\n创建地址: https://github.com/settings/tokens/new')
  if (!token) return

  const success = await authStore.loginWithToken(token.trim())
  if (success) {
    alert('登录成功！')
  } else {
    alert('登录失败，请检查 Token 是否正确')
  }
}
</script>